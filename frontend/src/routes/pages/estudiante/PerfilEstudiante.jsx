...
function PerfilEstudiante() {
  ...
  if (loading) {
    return (
      <>
        <PageHeader eyebrow="Panel estudiante" title="Mi perfil" />
  ...
  const temas = stats.temasPorReforzar ?? []
  ...
  return (
    <>
      <PageHeader
        ...
        title="Mi perfil"
        subtitle="Administra tu información y revisa tu progreso académico."
      />

      <div className={styles.stats}>
        <div className={styles.statCard}>
          <p className={styles.statValue}>{stats.cursosMatriculados ?? 0}</p>
          <p className={styles.statLabel}>Cursos matriculados</p>
        </div>
        <div className={styles.statCard}>
          <p className={styles.statValue}>{stats.promedioGeneral ?? 0}</p>
          <p className={styles.statLabel}>Promedio general (0–20)</p>
        </div>
        <div className={styles.statCard}>
          <p className={styles.statValue}>{stats.nivelActual ?? 'Básico'}</p>
          <p className={styles.statLabel}>Nivel actual</p>
        </div>
        <div className={styles.statCard}>
          <p className={styles.statValue}>{stats.evaluacionesRealizadas ?? 0}</p>
          <p className={styles.statLabel}>Evaluaciones realizadas</p>
        </div>
      </div>
      ...
      <div className={styles.layout}>
        <div className={styles.profileCard}>
          {perfil?.fotoPerfil ? (
            ...
            <span className={styles.avatar}>{getInitials(perfil?.nombres ?? 'E')}</span>
          )}
          <h2 className={styles.name}>{perfil?.nombres}</h2>
          <p className={styles.role}>Estudiante · {perfil?.correo}</p>
          ...
          <div style={{ marginTop: '18px', width: '100%' }}>
            <p className={styles.label}>Temas por reforzar</p>
            {temas.length > 0 ? (
              <div className={styles.topicList}>
                {temas.map((t) => (
                  <span key={t.tema} className={styles.topicTag}>
                    {t.tema} ({Math.round(t.porcentajeDominio)}%)
                  </span>
                ))}
              </div>
            ) : stats.evaluacionesRealizadas > 0 ? (
              <p className={styles.emptyText}>
                ¡Excelente! No tienes temas pendientes de refuerzo.
              </p>
            ) : (
              <p className={styles.emptyText}>
                Completa evaluaciones para ver tus temas por reforzar.
              </p>
            )}
          </div>
        </div>
